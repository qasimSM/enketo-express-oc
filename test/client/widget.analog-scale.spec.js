...
const FORM_SHOW_SCALE = `<label class="question non-select or-appearance-analog-scale or-appearance-vertical or-appearance-show-scale">
...
describe('Analog-scale widget with show-scale', () => {
    ...
    it('adds widget that contain visible min scale equals 0', () => {
        expect(
            widgetScalesContainer.querySelector('.range-widget__scale__start')
                .textContent
        ).to.equal('0');
    });
    ...
    it('adds widget that contain visible max scale equals 100', () => {
        expect(
            widgetScalesContainer.querySelector('.range-widget__scale__end')
                .textContent
        ).to.equal('100');
    });
    ...
    it('adds widget that contain 9 visible scales that not max and min', () => {
        expect(
            widgetScalesContainer.querySelectorAll(
                '.range-widget__scale__between'
            ).length
        ).to.equal(9);
    });

    it('adds widget that contain 9 visible scales that not max and min with the right values', () => {
        const scalesNotMinMax = widgetScalesContainer.querySelectorAll(
            '.range-widget__scale__between'
        );
        const scalesNotMinMaxValues = Array.from(scalesNotMinMax).map((scale) =>
            parseInt(scale.textContent, 10)
        );
        const expectedNotMinMaxValues = [10, 20, 30, 40, 50, 60, 70, 80, 90];

        expect(JSON.stringify(expectedNotMinMaxValues)).to.equal(
            JSON.stringify(scalesNotMinMaxValues)
        );
    });
});
