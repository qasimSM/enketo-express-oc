...
describe( 'Config Model', () => {
    ...
    describe( 'can be set using flat environment variables instead of config.json', () => {
        ...
        const localConfigPath = path.resolve( process.cwd(), './config/config.json' );
        ...
        it( 'for boolean values in a nested config item', () => {
            ...
            stubEnv( 'ENKETO_LINKED_FORM_AND_DATA_SERVER_LEGACY_FORMHUB', 'true' ); // string!
            config = loadConfig();
        ...
        it( 'parses a redis url to its components', () => {
            stubEnv( 'ENKETO_REDIS_MAIN_URL', 'redis://h:pwd@ec2-54-221-230-53.compute-1.amazonaws.com:6869' );
            config = loadConfig();
    ...
    describe( 'version', () => {
        ...
        beforeEach( () => {
            sandbox.stub( childProcess, 'execSync' ).callsFake( ( command, options ) => {
