import SuperWidget from 'enketo-core/src/js/widget';
...
function runAllCommonWidgetTests(
    Widget,
    template = FORM_CONTROL_TEMPLATE,
    value = '2',
    options = undefined
) {
...
function testRequiredMethods(Widget) {
    describe(`required methods for ${Widget.name}:`, () => {
        const widgetProtoValue = Object.getOwnPropertyDescriptor(
            Widget.prototype,
            'value'
        );
        const superProtoValue = Object.getOwnPropertyDescriptor(
            SuperWidget.prototype,
            'value'
        );
        ...
        it('passes options and populates widget props', (done) => {
            ...
            Promise.resolve()
                ...
                .then((widget) => {
                    ...
                    expect(widget.props.appearances.includes('one')).to.equal(
                        true
                    );
                    expect(widget.props.appearances.includes('two')).to.equal(
                        true
                    );
        ...
        it('adds an "ignore" class to all form controls inside the widget', (done) => {
            ...
            Promise.resolve()
                ...
                .then((widget) => {
                    if (widget.question) {
                        const widgetEl =
                            widget.question.querySelector('.widget');
                        if (widgetEl) {
                            let widgetControls = widgetEl.matches(
                                'input, select, textarea'
                            )
                                ? [widgetEl]
                                : [];
                            widgetControls = widgetControls.concat([
                                ...widgetEl.querySelectorAll(
                                    'input, select, textarea'
                                ),
                            ]);
                            expect(
                                widgetControls.every((el) =>
        ...
        it('works', (done) => {
            ...
            Promise.resolve()
                ...
                .then((widget) => {
                    ...
                    const resetButton = question
                        ? question.querySelector('.btn-reset')
                        : null;
                    if (resetButton) {
                        resetButton.click();
